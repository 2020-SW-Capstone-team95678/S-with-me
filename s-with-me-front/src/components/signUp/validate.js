export function validate(values, isCheck, isOnlyId) {
  ...
  if (userId && !isCheck) errors['userId'] = '아이디 중복 확인 검사를 해주세요';
  if (userId && isCheck && !isOnlyId) errors['userId'] = '중복되지 않은 아이디를 입력해 주세요';
  ...
  if (birthday && birthday.length !== 6)
    errors['birthday'] = '6자리의 생년월일을 입력해주세요. ex)001205';
  if (!phoneNumber) errors['phoneNumber'] = '휴대폰 번호를 입력해주세요';
